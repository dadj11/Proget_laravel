...
@section('content')
    <div class="space-y-5">
        ...
        <div class="  border border-gray-300 overflow-auto rounded rounded-xl ">
            <table class="w-full">
                <thead>
                    <tr class=" flex-col border-b border-gray-400  bg-gray-400  ">
                ...
                <tbody>
                    @forelse ($all as $one)
                        <tr class=" flex-col border-t border-gray-400 hover:bg-gray-100 space-y-5 py-3">
                            ...
                            <td class="px-4 py-3 flex  space-x-2">
                                @if ($one->delivery_status == 'PENDING')
                                    <form action="/admin/orders/{{ $one->id }}/reject" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <button class="bg-red-500 hover:bg-red-400 rounded rounded-sm text-white px-2"
                                            type="submit">REJECT</button>
                                    </form>
                                    <form action="/admin/orders/{{ $one->id }}/start-livraison" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <button class="bg-blue-500 hover:bg-blue-400 rounded rounded-sm text-white px-2"
                                ...
                                @if ($one->delivery_status == 'START')
                                    <form action="/admin/orders/{{ $one->id }}/confirm-livraison" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <button class=" bg-green-500 hover:bg-green-400 rounded rounded-sm text-white px-2"
