        ...
        <div class="border border-gray-400 rounded rounded-3xl p-6">
            ...
            <form action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <input type="text" name="label" id="libele" class="border border-gray-400 rounded rounded-1xl"
                        value="{{ old('label', $article->label) }}">
                </div>

                <div>
                    <label for="quantite"> Quantite</label>
                    <input type="number" class="border border-gray-400 rounded rounded-1xl" name="quantite" id="quantite"
                        value="{{ old('quantite', $article->quantite) }}">
                </div>

                <div>
                    <label for="prix">Prix unitaire</label>
                    <input type="number" class="border border-gray-400 rounded rounded-1xl " name="prix" id="prix"
                        value="{{ old('current_price', $article->current_price) }}">
                </div>

                <div class=" flex flex-col">
                    <label for="description">Description</label>
                    <textarea name="description" id="description" cols="30" class="border border-gray-400 rounded rounded-1xl "
